...
import Day14.Ex01_Board.DTO.Text;
...
public class CommentDAO extends JDBConnection implements DataService {
	...
	public int insert(Text text) {
		int result = 0;
		String sql = " INSERT INTO comment( board_no, writer, content) " + " VALUES( ?, ? ,? ) ";

		System.out.println("댓글 작성 : boardNo" + text.getNo());

		try {
			psmt = con.prepareStatement(sql); // 쿼리 실행 객체 생성
			psmt.setInt(1, text.getNo()); // 1번 ? 에 댓글 번호로 매핑
		...
		} catch (SQLException e) { // 댓글 1개 쓰기 성공 시, result : 1
			System.err.println("댓글 등록 시, 에러 발생"); // 실패 시, result : 0
			e.printStackTrace();
		}
		return result;
	}
	...
	public int update(Text text) {
		...
		int result = 0;
		String sql = " UPDATE comment " + " SET writer = ? " + "	  ,content = ? " + "	  ,upd_date = now() "
				+ " WHERE comment_no = ? ";
